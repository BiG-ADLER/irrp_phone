...
function notifyAlertSMS (number, alert, listSrc)
  ...
  if type(alert.coords) == "boolean" then
    return
  end
    ...
    for k, _ in pairs(listSrc) do
      getPhoneNumber(tonumber(k), function (n)
        if n ~= nil then
          ...
          local doeshavephone
          if phone ~= nil and phone.count > 0 then doeshavephone = true else doeshavephone = false end

            TriggerEvent('gcPhone:_internalAddMessagemgrp', number, n, 'From #' .. alert.numero  .. ' : ' .. alert.message, 0, function (smsMess)
            TriggerClientEvent("irrp_phone:receiveMessage", tonumber(k), smsMess, doeshavephone)
            print('2')
            TriggerEvent('gcPhone:_internalAddMessagemgrp', number, n, 'GPS: ' .. alert.coords.x .. ', ' .. alert.coords.y, 0, function (smsMess)
            TriggerClientEvent("irrp_phone:receiveMessage", tonumber(k), smsMess, false)
            print('1')
          end)
          end)
        end
...
RegisterServerEvent('esx_addons_gcpirrphone:startCairrpll')
AddEventHandler('esx_addons_gcpirrphone:startCairrpll', function (number, message, coords)
